...
Given /^an eml document with read access given to "([^"]*)" in the root element is inserted by "([^"]*)"$/ do |reader, owner|
  ...
  resource = RestClient::Resource.new('http://pasta.lternet.edu/gatekeeper/package/eml', :user=>"uid=#{owner},o=LTER,dc=ecoinformatics,dc=org", :password => get_password_for(owner))
...
Given /^I read the document as "([^"]*)"$/ do |user|
  resource = RestClient::Resource.new("http://pasta.lternet.edu/gatekeeper/package/eml/#{@eml.scope}/#{@eml.id}/#{@eml.rev}", :user=>"uid=#{user},o=LTER,dc=ecoinformatics,dc=org", :password => get_password_for(user))
